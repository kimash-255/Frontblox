...
const Shortcut = ({ id, initialTitle = "Shortcut Title", linkType = "", linkTo = "", initialPosition = { left: 0, top: 0 }, onSave, onDelete }) => {
  const [title, setTitle] = useState(initialTitle);
  const [link, setLink] = useState(linkTo);
  const [linkTypeState, setLinkTypeState] = useState(linkType);
  ...
  const handleLinkTypeChange = (e) => {
    setLinkTypeState(e.target.value);
  };

  const handleSave = () => {
    onSave(id, title, linkTypeState, link, position);
    setIsEditing(false);
  };
    ...
    <div className="relative p-4 border border-gray-300 rounded shadow-lg bg-gradient-to-r from-pink-500 to-purple-500">
      {isEditing ? (
        ...
        <div>
          ...
          {linkTypeState && link && (
            <a href={link} className="text-white underline mt-2 block">
              {linkTypeState}: {link}
            </a>
          )}
        </div>
