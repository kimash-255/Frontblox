...
const Heading = ({ id, initialText = "Default Heading", initialPosition = "left", onSave, onDelete, onPositionChange }) => {
  ...
  const handlePositionChange = (newPosition) => {
    ...
    if (onPositionChange) onPositionChange(id, newPosition); // Check if function exists
  };
      ...
      {isEditing ? (
        <>
          <input
            ...
            className={`text-2xl font-bold text-gray-800 border-none outline-none bg-transparent mb-2 w-full text-${position}`}
        ...
        <h1 className={`text-2xl font-bold text-gray-800 text-${position}`}>
          {text}
        </h1>
      )}
